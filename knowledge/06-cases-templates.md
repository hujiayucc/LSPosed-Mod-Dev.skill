...
真实项目案例用于提炼工程结构、API 使用方式、Hook 分层、日志、排错、兼容设计、静态分析和动态调试模式。可以结合反编译代码、调用链、协议字段、Native 符号和运行日志分析具体实现，但不要机械照搬第三方项目业务逻辑；案例结论需标注版本、证据来源和待验证假设。

涉及检测、反作弊、风控、隐藏注入或敏感业务机制时，先区分静态分析、动态观测、兼容修复与行为验证，再给出最小 Hook、可观测日志、失败回退和验证路径。
...
案例材料应标注：具体业务逻辑、敏感 Hook、过时 API、项目特定假设、分析证据和版本差异；涉及敏感数据时使用脱敏样本，不把真实凭据、个人数据或设备标识写入日志和模板。
...
Skill 必须能从真实项目和 APP 分析材料中提炼通用开发模式，区分官方基准、现代实战、旧 API 迁移、静态证据、动态证据和低质量示例；能分析 APK、DEX、smali、Native so、调用链、协议和运行日志；能设计清晰目录结构，把入口、Hook、Guard、Logger、State、Service 分层；能生成 API 102 优先的 Java/Kotlin 模板；能给出可执行排错顺序；能审查 `module.prop`、`java_init.list`、`scope.list`；能识别 Hook 时机、ClassLoader、进程、scope、方法签名问题；能建议最小 Hook 点、可观测日志和安全回退策略；能避免把旧 API 示例误用为现代最佳实践；能对高风险运行时路径给出版本、崩溃保护、恢复和验证步骤。